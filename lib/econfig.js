...
const ExitCodes = {
	...
	EXIT_CODE_INIT: 66,
	EXIT_CODE_MISSING_OR_INVALID_ENV_PROPERTY: 67,
	EXIT_CODE_CONFIGURATION_ERROR: 68
};
...
/**
 * In case a property is required or optional for a certain environment we can handle that with the EOptionalForEnvironment object
 *
...
class EConfigTemplate {
	constructor(envPrefix = undefined, /* istanbul ignore next */ test = false) {
	...
	newPropertyInternal(defaultSet, envName, validator, defaultValue, replaceEnv, mustEnv) {
		...
		let prop;
		if (defaultSet)
			prop = new EConfigProperty(envName, defaultValue);
		else
			prop = new EConfigProperty(envName);
		...
		if (propsMissing || propsInvalid) {
			...
			if (exitOnError) {
				...
				if (typeof process.exit === "function")
					process.exit(ExitCodes.EXIT_CODE_MISSING_OR_INVALID_ENV_PROPERTY);
				else
					throw new Error("ExitCodes.EXIT_CODE_MISSING_OR_INVALID_ENV_PROPERTY");
			} else {
				return {
